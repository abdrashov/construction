<template>
    <div>
            ...
            <form @submit.prevent="update">
                <div class="items-start lg:flex">
                    <div class="flex flex-wrap px-4 py-3 w-full">
                        <text-input v-model="form_invoice.name" :error="form_invoice.errors.name" class="pb-4 pr-0 w-full lg:pr-4 lg:w-1/2" label="Номер" />

                        <div class="pb-4 w-full lg:w-1/2">
                            <label class="form-label">Дата:</label>
                            <date-picker v-model="form_invoice.date" mode="date" is24hr :masks="{ input: 'YYYY-MM-DD' }">
                ...
                <div class="flex items-center px-5 py-3 bg-gray-50 border-t border-gray-100">
                    ...
                    <loading-button :loading="form.processing" class="btn-green ml-auto" type="submit">Обновить Накладной</loading-button>
                </div>
            </form>
        </div>
                ...
                <tr v-for="(item, index) in invoice_items" :key="item.id">
                    ...
                    <td class="w-1/6 border-l border-t">
                        <div class="flex items-center px-4 py-1">
                            <input type="number" v-model.lazy="item.count" @change="countHandler(index)" :name="`item[${index}][count]`" class="mr-1 border-b border-dashed border-blue-300" />
                            <span>{{ item.measurement }}</span>
                        </div>
                    </td>
                    <td class="w-1/6 border-l border-t">
                        <div class="flex items-center px-4 py-1">
                            <input type="number" v-model.lazy="item.price" @change="priceHandler(index)" :name="`item[${index}][price]`" class="border-b border-dashed border-blue-300" />
                        </div>
                    </td>
        ...
        <div class="flex items-center justify-between mb-3 mt-6 text-xl">
            <div class=""></div>
            <button @click.prevent="confirm()" type="submit" class="btn-cyan mb-1 mr-2">
