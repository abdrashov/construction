            ...
            <table class="w-full">
                ...
                <tr v-for="(invoice, index) in invoices" :key="invoice.id" class="duration-150 hover:bg-amber-50 focus:bg-amber-50">
                    ...
                    <td class="border-t border-l">
                        <Link class="flex items-center px-4 py-1 font-medium hover:underline" :href="`/reports/${organization.id}/${supplier.id}/${invoice.id}/items?old=not_pay`+(form.begin ? '&begin='+form.begin : '')+(form.end ? '&end='+form.end : '')">
                            {{ invoice.name }}
                        </Link>
                    </td>
                    <td class="border-t border-l">
                        <Link class="flex items-center px-4 py-1 hover:underline" :href="`/reports/${organization.id}/${supplier.id}/${invoice.id}/items?old=not_pay`+(form.begin ? '&begin='+form.begin : '')+(form.end ? '&end='+form.end : '')">
                            {{ invoice.accepted }}
                        </Link>
                    </td>
                    <td class="w-2/6 border-t border-l">
                        <Link class="flex items-center px-4 py-1 hover:underline whitespace-nowrap" :href="`/reports/${organization.id}/${supplier.id}/${invoice.id}/items?old=not_pay`+(form.begin ? '&begin='+form.begin : '')+(form.end ? '&end='+form.end : '')">
    ...
    watch: {
        form: {
            ...
            handler: throttle(function () {
                this.$inertia.get(`/reports/${this.organization.id}/${this.supplier.id}/not_pay`, pickBy(this.form), { preserveState: true })
            }, 150),
        },
    },
    methods: {
